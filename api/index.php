...
function db(): PDO {
    ...
    if (!$pdo) {
        ...
        $pdo->exec("CREATE TABLE IF NOT EXISTS workspace_data (workspace_id INT UNSIGNED PRIMARY KEY, projects_json LONGTEXT NOT NULL, CONSTRAINT fk_workspace_data_workspace FOREIGN KEY (workspace_id) REFERENCES workspaces(id) ON DELETE CASCADE) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci");
...
try {
    ...
    if ($action === 'tasks') {
        ...
        $sql=$canSeeAll ? 'SELECT id,owner_id,payload_json,updated_at FROM tasks WHERE workspace_id=? ORDER BY updated_at DESC' : 'SELECT id,owner_id,payload_json,updated_at FROM tasks WHERE workspace_id=? AND owner_id=? ORDER BY updated_at DESC';
        $q=db()->prepare($sql); $canSeeAll ? $q->execute([$wid]) : $q->execute([$wid,$u['id']]); $tasks=[]; foreach($q as $r){$item=json_decode($r['payload_json'],true)?:[];$item['id']=$r['id'];$item['_owner_id']=$r['owner_id'];$tasks[]=$item;}
