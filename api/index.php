...
function db(): PDO {
    ...
    if (!$pdo) {
        ...
        try { $pdo->exec("ALTER TABLE push_jobs ADD COLUMN subscription_hash CHAR(64) NULL AFTER user_id"); } catch (Throwable $e) {}
        try { $pdo->exec("ALTER TABLE push_jobs ADD INDEX idx_push_subscription (subscription_hash)"); } catch (Throwable $e) {}
...
function sendWebPush(array $subscription, string $payload, array $config): array {
    ...
    // Un aviso de Pomodoro pierde utilidad rápidamente. Un TTL corto evita que
    // Apple/Google lo entreguen muchos minutos después al volver a conectarse.
    $ch=curl_init($url); curl_setopt_array($ch,[CURLOPT_POST=>true,CURLOPT_POSTFIELDS=>$body,CURLOPT_RETURNTRANSFER=>true,CURLOPT_TIMEOUT=>15,CURLOPT_HTTPHEADER=>['TTL: 120','Urgency: high','Content-Type: application/octet-stream','Content-Encoding: aes128gcm','Authorization: vapid t='.$jwt.', k='.$config['vapid_public_key']]]); $response=curl_exec($ch); $code=(int)curl_getinfo($ch,CURLINFO_HTTP_CODE); $error=curl_error($ch); curl_close($ch); return ['ok'=>$code>=200&&$code<300,'code'=>$code,'error'=>$error?:substr((string)$response,0,200)];
}
...
try {
    ...
    if ($action === 'push_run') {
        ...
        foreach($jobs as $job){
            // No entregar avisos antiguos si el cron o el proveedor estuvo
            // detenido. Es preferible omitirlos a interrumpir tarde al usuario.
            if(time()-(int)$job['fire_at']>180){$pdo->prepare('DELETE FROM push_jobs WHERE id=?')->execute([$job['id']]);continue;}
            $subHash=(string)($job['subscription_hash']??'');
            // Los trabajos viejos no indican en qué dispositivo nacieron. No
            // se difunden: eso era precisamente lo que causaba avisos cruzados.
            if($subHash===''){$pdo->prepare('DELETE FROM push_jobs WHERE id=?')->execute([$job['id']]);continue;}
            $q=$pdo->prepare('SELECT * FROM push_subscriptions WHERE user_id=? AND endpoint_hash=?');$q->execute([(int)$job['user_id'],$subHash]);
            $payload=json_encode(['title'=>$job['title'],'body'=>$job['body'],'url'=>'./','tag'=>'kaizen-'.$job['job_key'],'timestamp'=>(int)$job['fire_at']*1000],JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES);
            foreach($q as $sub){$result=sendWebPush($sub,$payload,$config);if($result['ok'])$sent++;else{$failed++;if(in_array((int)$result['code'],[404,410],true))$pdo->prepare('DELETE FROM push_subscriptions WHERE id=?')->execute([$sub['id']]);}}
            $pdo->prepare('DELETE FROM push_jobs WHERE id=?')->execute([$job['id']]);
        }
    ...
    if ($action === 'push_subscribe') {
        $endpoint=trim((string)($data['endpoint']??''));$keys=is_array($data['keys']??null)?$data['keys']:[];$p256dh=trim((string)($keys['p256dh']??''));$auth=trim((string)($keys['auth']??''));$deviceLabel=substr(trim((string)($data['device_label']??'Navegador')),0,80) ?: 'Navegador';if($endpoint===''||$p256dh===''||$auth==='')out(['ok'=>false,'error'=>'Suscripción de push incompleta.'],422);$pdo=db();$subHash=hash('sha256',$endpoint);$q=$pdo->prepare('INSERT INTO push_subscriptions(user_id,endpoint_hash,endpoint,p256dh,auth,device_label) VALUES(?,?,?,?,?,?) ON DUPLICATE KEY UPDATE user_id=VALUES(user_id),endpoint=VALUES(endpoint),p256dh=VALUES(p256dh),auth=VALUES(auth),device_label=VALUES(device_label)');$q->execute([(int)$u['id'],$subHash,$endpoint,$p256dh,$auth,$deviceLabel]);
        // Retira trabajos creados por versiones antiguas, que no distinguían
        // dispositivos y podían avisar en todos a la vez.
        $pdo->prepare('DELETE FROM push_jobs WHERE user_id=? AND subscription_hash IS NULL')->execute([(int)$u['id']]);$c=$pdo->prepare('SELECT COUNT(*) FROM push_subscriptions WHERE user_id=?');$c->execute([(int)$u['id']]);out(['ok'=>true,'registered'=>(int)$c->fetchColumn(),'subscription_hash'=>$subHash]);
    }
    if ($action === 'push_schedule') {
        $jobKey=preg_replace('/[^a-zA-Z0-9_-]/','',substr((string)($data['job_id']??''),0,80));$fireAt=(int)($data['fire_at']??0);$title=trim((string)($data['title']??'Pomodoro terminado'));$bodyText=trim((string)($data['body']??'Terminaste un ciclo de enfoque.'));$subHash=strtolower(trim((string)($data['subscription_hash']??'')));if($jobKey===''||$fireAt<time()-60||!preg_match('/^[a-f0-9]{64}$/',$subHash))out(['ok'=>false,'error'=>'Programación de push inválida.'],422);$pdo=db();$q=$pdo->prepare('SELECT 1 FROM push_subscriptions WHERE user_id=? AND endpoint_hash=?');$q->execute([(int)$u['id'],$subHash]);if(!$q->fetchColumn())out(['ok'=>false,'error'=>'Este dispositivo no está suscrito.'],422);$pdo->prepare('INSERT INTO push_jobs(user_id,subscription_hash,job_key,fire_at,title,body) VALUES(?,?,?,?,?,?) ON DUPLICATE KEY UPDATE subscription_hash=VALUES(subscription_hash),fire_at=VALUES(fire_at),title=VALUES(title),body=VALUES(body)')->execute([(int)$u['id'],$subHash,$jobKey,$fireAt,substr($title,0,180),$bodyText]);out(['ok'=>true]);
    }
    if ($action === 'push_cancel') { $jobKey=preg_replace('/[^a-zA-Z0-9_-]/','',substr((string)($data['job_id']??''),0,80));$subHash=strtolower(trim((string)($data['subscription_hash']??'')));if($jobKey!==''&&preg_match('/^[a-f0-9]{64}$/',$subHash))db()->prepare('DELETE FROM push_jobs WHERE user_id=? AND subscription_hash=? AND job_key=?')->execute([(int)$u['id'],$subHash,$jobKey]);out(['ok'=>true]); }
    if ($action === 'push_test') {
        $subHash=strtolower(trim((string)($data['subscription_hash']??'')));if(!preg_match('/^[a-f0-9]{64}$/',$subHash))out(['ok'=>false,'error'=>'No se identificó este dispositivo.'],422);$q=db()->prepare('SELECT * FROM push_subscriptions WHERE user_id=? AND endpoint_hash=?'); $q->execute([(int)$u['id'],$subHash]); $subs=$q->fetchAll();
        $payload=json_encode(['title'=>'Aviso de prueba de Kaizen','body'=>'Las notificaciones push están activas en este dispositivo.','url'=>'./','tag'=>'kaizen-test-'.time(),'timestamp'=>time()*1000],JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES);
